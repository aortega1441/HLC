...
public class MaquinaDeGolosinas {
	...
	private void maquinaDeGolosinas(String[][] golosinas, double[][] precioGolosinas, int[][] cantidadGolosinas,
			double[][] ventas) {
		...
		while (salirMenu == false) {
			...
			case 1:
				pideGolosina(cantidadGolosinas, golosinas, precioGolosinas, sc, ventas);
				break;
			case 2:
				muestraGolosina(golosinas, precioGolosinas, cantidadGolosinas);
				break;
			case 3:
				rellenaGolosinas(cantidadGolosinas, golosinas, sc);
				break;
			case 4:
				apagaMaquinaYGuardaCambios(golosinas, cantidadGolosinas, precioGolosinas, ventas);
				salirMenu = true;
				break;
			default:
				System.err.println("Introduzca un opción correcta.");
				break;
			}
		}
	}

	private void apagaMaquinaYGuardaCambios(String golosinas[][], int cantidadGolosinas[][], double precioGolosinas[][],
			double[][] ventas) {
	...
	private void muestraGolosina(String[][] golosinas, double[][] precioGolosinas, int[][] cantidadGolosinas) {
		// TODO Auto-generated method stub
		System.out.printf("%-24s %-5s %s %n", "  Golosina", "Cod.", "Precio");
		System.out.println();
		for (int x = 0; x < golosinas.length; x++) {
			for (int y = 0; y < golosinas[x].length; y++) {
	...
	private void pideGolosina(int[][] cantidadGolosinas, String[][] golosinas, double[][] precioGolosinas, Scanner sc,
			double[][] ventas) {
		// TODO Auto-generated method stub
		...
		try {
			...
			if (cantidadGolosinas[fil][col] > 0) {
				...
				System.out.println("Has recibido " + "'" + golosinaComprada + "'");
				System.out.println();
				ventas[fil][col] += precioGolosinas[fil][col];
				cantidadGolosinas[fil][col]--;
				datoCorrecto = true;
			} else
				System.err.println("No quedan más golosinas de este tipo");
			datoCorrecto = true;
		} catch (Exception e) {
			System.err.println("Código erroneo.");
			System.out.println();
